...
openssl req -new -key root_cert_key.pem -subj "/CN=Mock_IoT_Test_Root" -out root_req.pem

openssl x509 -req \
    -in root_req.pem \
    -extfile mock_iot_certs.conf -extensions root_cert \
...
# mock-iot-server always uses hostname localhost, so its server certificate must always have CN or SAN localhost.
openssl req -new -key server_cert_key.pem -subj "/CN=localhost" -out server_req.pem

openssl x509 -req \
    -in server_req.pem \
    -extfile mock_iot_certs.conf -extensions server_cert \
