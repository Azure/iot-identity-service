...
async fn main() -> Result<(), Error> {
    let config_path: std::path::PathBuf = std::env::var_os("AZIOT_TPMD_CONFIG")
        .map_or_else(|| "/etc/aziot/tpmd/config.toml".into(), Into::into);

    let config = std::fs::read(config_path).map_err(|err| {
        aziot_tpmd::Error::Internal(aziot_tpmd::InternalError::ReadConfig(Box::new(err)))
    })?;
    let aziot_tpmd::Config {
        endpoints: aziot_tpmd::Endpoints {
    ...
    let server = aziot_tpmd::Server::new()?;
