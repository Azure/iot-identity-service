...
pub(crate) struct Server {
	pub(crate) inner: std::sync::Arc<csd::Server>,
}

/// A route is an async function that receives the hyper request and the `ksd::Server` value.
///
/// It returns `Ok(res)` if it successfully matched the incoming request, and `Err(req)` if it didn't.
type Route =
	fn(
		hyper::Request<hyper::Body>,
		std::sync::Arc<csd::Server>,
