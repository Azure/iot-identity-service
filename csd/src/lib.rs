...
pub struct Server {
	...
	ks_engine: std::sync::Arc<std::sync::Mutex<openssl2::FunctionalEngine>>,
}

impl Server {
	pub fn new(
		homedir_path: std::path::PathBuf,
		ks_client: std::sync::Arc<ks_client::Client>,
	) -> Result<Self, Error> {
		let ks_engine = openssl_engine_ks::load(ks_client).map_err(|err| Error::Internal(InternalError::LoadKeysServiceOpensslEngine(err)))?;
		let ks_engine = std::sync::Arc::new(std::sync::Mutex::new(ks_engine));
...
impl Server {
	pub fn create_cert(
		...
		if let Some((issuer_id, issuer_private_key)) = issuer {
			...
			let mut ks_engine = self.ks_engine.lock().expect("ks engine mutex poisoned");
			let ks_engine = &mut *ks_engine;
			...
			let issuer_private_key =
				ks_engine.load_private_key(&issuer_private_key).map_err(|err| Error::Internal(InternalError::CreateCert(Box::new(err))))?;
