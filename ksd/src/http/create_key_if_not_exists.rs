...
pub(super) fn handle(
	...
	Box::pin(async move {
		...
		let create_key_value = match (body.generate_key_len, body.import_key_bytes) {
			...
			(None, Some(import_key_bytes)) => {
				let import_key_bytes = match base64::decode(&import_key_bytes) {
					Ok(import_key_bytes) => import_key_bytes,
					Err(err) => return Ok(super::err_response(
						hyper::StatusCode::UNPROCESSABLE_ENTITY,
						None,
						super::error_to_message(&err).into(),
				...
				ks_common::CreateKeyValue::Import { bytes: import_key_bytes }
			},
