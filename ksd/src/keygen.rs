...
pub struct KeyGenRawError(pub(crate) sys::KEYGEN_ERROR);

impl std::fmt::Display for KeyGenRawError {
...
impl KeyGen {
	pub(crate) fn create_key_pair_if_not_exists(
		...
		unsafe {
			match self {
				KeyGen::V2_0_0_0 { create_key_pair_if_not_exists, .. } => {
					keygen_fn(|| create_key_pair_if_not_exists(
						id.as_ptr(),
...
impl KeyGen {
	pub(crate) fn load_key_pair(
		...
		unsafe {
			match self {
				KeyGen::V2_0_0_0 { load_key_pair, .. } => {
					keygen_fn(|| load_key_pair(
						id.as_ptr(),
					...
					match parameter_name {
						...
						parameter_name => {
							...
							let parameter_type = match parameter_name {
								...
								_ => return Err(GetKeyPairPublicParameterError::Api { err: KeyGenRawError(sys::KEYGEN_ERROR_INVALID_PARAMETER) }),
							};
...
impl KeyGen {
	pub(crate) fn create_key_if_not_exists(
		...
		unsafe {
			match self {
				KeyGen::V2_0_0_0 { create_key_if_not_exists, .. } => {
					keygen_fn(|| create_key_if_not_exists(
						id.as_ptr(),
						length,
...
impl KeyGen {
	pub(crate) fn import_key(
		...
		unsafe {
			match self {
				KeyGen::V2_0_0_0 { import_key, .. } => {
					keygen_fn(|| import_key(
						id.as_ptr(),
						bytes.as_ptr(),
						bytes.len(),
			...
			match self {
				KeyGen::V2_0_0_0 { sign, .. } => {
					...
					if signature_len > signature.len() {
						// libiothsm-keygen scribbled past the end of the buffer. Crash as soon as possible.
						std::process::abort();
					}
...
fn keygen_fn(f: impl FnOnce() -> sys::KEYGEN_ERROR) -> Result<(), KeyGenRawError> {
	match f() {
		sys::KEYGEN_SUCCESS => Ok(()),
		err => Err(KeyGenRawError(err)),
	}
}
