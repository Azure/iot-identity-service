...
//!
//! To use the engine, obtain a [`ks_client::Client`] and call [`load`]
...
pub fn load(client: std::sync::Arc<ks_client::Client>) -> Result<openssl2::FunctionalEngine, openssl2::Error> {
	unsafe {
...
openssl_errors::openssl_errors! {
	...
	library Error("openssl_engine_ks") {
		functions {
			...
			KS_EC_SIGN("ks_ec_sign");

			KS_RSA_SIGN("ks_rsa_sign");
		}
	...
	match f() {
		...
		Err(err) => {
			...
			if let Some(function) = function {
				...
				eprintln!("[openssl-engine-ks] error: {}", err);
			}
			...
			while let Some(err) = source {
				if let Some(function) = function {
					...
					eprintln!("[openssl-engine-ks] caused by: {}", err);
				}
