...
pub(super) struct Engine {
	client: std::sync::Arc<ks_client::Client>,
}

impl Engine {
	pub(super) unsafe fn load(client: std::sync::Arc<ks_client::Client>) -> Result<openssl2::FunctionalEngine, openssl2::Error> {
		const ENGINE_ID: &[u8] = b"openssl-engine-ks\0";
...
unsafe extern "C" fn ks_dupf_engine_ex_data(
...
#[no_mangle]
#[allow(clippy::similar_names)]
unsafe extern "C" fn ks_freef_engine_ex_data(
...
unsafe extern "C" fn engine_load_privkey(
	...
	let result = super::r#catch(Some(|| super::Error::ENGINE_LOAD_PRIVKEY), || {
		...
		let key_handle = ks_common::KeyHandle(key_handle.to_owned());
